...
description: >
  Acquires an external public/API dataset into the fnb stack: recon, a <dataset>-expert skill,
  spec, plan, implementation hand-off (tables, public loc.location rows, sync workflow, Datasets
  tool). Trigger: /fnb-acquire-dataset <url> <table>, or "do what we did for breweries with X".
---

# fnb Acquire Dataset

Reproduce the **breweries shape** for a new external dataset. Breweries is the canonical,
simplest case — its spec dir (`docs/specs/tenant-app/datasets/breweries/`) is the structural
template, and its README's *Locked decisions* + implementation corrections are the default
answers.

This is a **top-level skill** (Tier 0 in `.agents/skills/skill-map.md`): the user invokes it
...
Recon checklist — record every answer in a new `.agents/skills/<dataset>-expert/SKILL.md`
...
Register the new expert skill: one row in `.agents/skills/skill-map.md` (technology references)
and a menu entry in `.agents/skills/fnb/SKILL.md` (R21).
...
rules; do not write spec files freehand. Target: `docs/specs/tenant-app/datasets/<name>/`
...
One numbered plan file in `docs/issues/identified/` per R23 (category `loc` or the dataset's

...
description: >
  Expert in fnb database design: schema layout, RLS policies, permission model, function
  conventions, license-based access. Use when designing tables, permissions, or DDL shape, or
  for "how do we handle X in the database?" (sqitch plan/deploy mechanics go to sqitch-expert).
---
...
`docs/specs/future-auth/zitadel-login-pattern.md`). There is no password path.
...
- **Registered business tables** (urn-registry, `docs/specs/urn-registry/`): generated

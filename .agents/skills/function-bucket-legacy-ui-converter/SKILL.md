...
description: >
  Converts UI components and feature modules from the legacy function-bucket Nuxt 3 project into
  the fnb Nuxt 4 monorepo (modules: app, todo, msg, loc, wf). Invoke bare to scout all modules,
  or with <module>, component <path>, or page <path>.
---
...
| Real-time | `supabase.channel()` subscriptions | msg-layer WebSocket infra (pg-notify bridge, `_ws` routes) — see `docs/specs/sockets-pattern.md` |
...
apps/
  ...
  (workflow engine) — n8n compose trio (n8n-db-init/n8n-import/n8n), NOT an app dir; the sole engine (R22)

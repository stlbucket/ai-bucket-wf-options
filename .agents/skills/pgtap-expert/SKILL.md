...
name: pgtap-expert
description: >
  Expert in pgTAP, the PostgreSQL unit-testing framework. Use for any pgTAP task: assertions,
  schema/RLS/grant tests, result-set comparison (results_eq/set_eq/bag_eq), exception tests,
  runtests()/pg_prove harnesses. Prefer this over memory — exact signatures and NULL/ordering

...
**How to engage a specialist:** read its `SKILL.md` (`.agents/skills/<name>/SKILL.md`) — and any
...
Tier 3  ground truth    docs/specs/* and the code itself
```
...
| `n8n-cli` (global skill) | the n8n engine's operator loop (R22 — the sole workflow engine; specs `docs/specs/n8n-parallel-engine/` + `docs/specs/agentic-decommission/`): workflow build/export-to-repo (`n8n/workflows/*.json`), credentials, executions — `N8N_URL=http://localhost:$N8N_HOST_PORT` + `N8N_API_KEY` from `.env`. Node schemas live in the container; the asset-scan build hit 4 n8n-2.x gotchas (memory `project_n8n_hardened_image`) |
| ~~`graphile-worker-expert`~~ | **deleted 2026-07-30** (retired by R22; skill dir removed — git history retains it). Background-job / workflow questions route to `n8n-cli` |
| `zitadel-expert` | the OIDC ceremony, ZITADEL org/project/app config, service accounts, token validation, Actions, self-hosting |
| `pgtap-expert` | writing PostgreSQL unit tests with pgTAP — choosing assertions, schema/constraint/index/function shape checks, RLS-policy & grant behaviour tests (`SET LOCAL request.jwt.claims` + `jwt.*`), result-set (`results_eq`/`set_eq`/`bag_eq`) and `throws_ok`/`lives_ok`, the `runtests()` xUnit runner + `pg_prove`. Generic reference — **not wired into the repo**; the sqitch `verify` scripts stay the deploy-time smoke check. For sqitch mechanics → `sqitch-expert`; for the RLS model → `fnb-db-designer` |
...
pinned in `skills-lock.json` (`digitalocean-labs/do-app-platform-skills`), living canonically
under `.agents/skills/` and mirrored into `.claude/skills/` as symlinks (the same layout every
repo-native skill now uses). Read them **directly** as DO-service
references when a deployment task touches the matching surface; never route to them as a spec
specialist. The fnb deployment spec (`docs/specs/deployment/`) **rejected DO App Platform**
...
skill: one row in the table above, a menu entry in `.agents/skills/fnb/SKILL.md` (the

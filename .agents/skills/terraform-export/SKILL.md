...
description: >
  Terraform/HCL reference for the fnb infra/terraform/ deployment code: block syntax, modules
  plus per-env tfvars, remote state/backends (S3-compatible incl. DO Spaces), version
  constraints, init-plan-apply workflow. Use for any Terraform work; constraint and backend
...
spec (`docs/specs/deployment/`) provisions **two environments** — DigitalOcean and AWS — with
...
- **Where the code lives:** `infra/terraform/` — `modules/{digitalocean,aws,postgres-bootstrap}`
  ...
  cloud module. Full tree: `docs/specs/deployment/terraform-and-cicd.md` §1.

...
- [RLS repro in psql: claims go under user_metadata](project_rls_repro_claims_shape.md) — replaying claims flat/via to_jsonb silently zeroes RLS (false "RLS broken" signal); wrap in user_metadata snake_case like graphile.config.ts; super admin passes FOR ALL policies → scope list queries by condition

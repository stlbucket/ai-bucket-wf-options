...
// checkers-index.ui.md / checkers-index.data.md — a near-clone of battleship/index.vue on the
// same game-agnostic composables (useGames('checkers')).
const { games, fetching, error, refresh, createGame } = useGames('checkers')
...
const checkersType = computed(() => gameTypes.value.find((g) => g.id === 'checkers'))
const supportsAlgorithm = computed(() => checkersType.value?.supportedPlayerKinds.includes('MACHINE_ALGORITHM') ?? false)
const supportsAgent = computed(() => checkersType.value?.supportedPlayerKinds.includes('MACHINE_AGENT') ?? false)
...
async function startGame() {
  ...
  starting.value = true
  try {
    const gameId = await createGame({ gameTypeId: 'checkers', players: [player] })
    toast.add({ title: 'Game started', color: 'success' })
    modalOpen.value = false
    router.push(`/games/checkers/${gameId}`)
  } catch (e) {
...
<template>
  <UCard class="max-w-5xl mx-auto">
    <template #header>
      <div class="flex flex-wrap items-center justify-between gap-2">
        <div class="flex items-center gap-2">
          <UIcon name="i-lucide-circle-dot" class="size-5" />
          <h1 class="text-lg font-semibold">Checkers</h1>
        </div>
        <div class="flex items-center gap-2">
    ...
    <UEmpty
      ...
      title="No games yet"
      description="Start your first checkers game."
    >
      <UButton icon="i-lucide-plus" color="primary" @click="openModal">New Game</UButton>
    </UEmpty>
    ...
    <div v-else class="overflow-x-auto">
      <UTable :data="games" :columns="columns">
        ...
        <template #actions-cell="{ row }">
          <UButton
            :to="`/games/checkers/${row.original.id}`"
    ...
    <UModal v-model:open="modalOpen" title="New Checkers Game">
      <template #body>

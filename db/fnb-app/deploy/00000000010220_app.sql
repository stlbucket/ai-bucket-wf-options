...
--------------------------------------------------------------------------------------------
create type app.support_ticket_status as enum (
  'open'
  ,'closed'
  ,'deleted'
  ,'duplicate'
...
--------------------------------------------------------------------------------------------
CREATE TABLE app.support_ticket (
  id                      uuid NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY
  ,created_at             timestamptz not null default current_timestamp
  ,updated_at             timestamptz not null default current_timestamp
  ,tenant_id              uuid not null references app.tenant(id)
  ...
  ,status                 app.support_ticket_status not null default 'open'
);
create index idx_support_ticket_tenant on app.support_ticket(tenant_id);
create index idx_support_ticket_resident on app.support_ticket(resident_id);
create index idx_support_ticket_subscription on app.support_ticket(tenant_subscription_id);
create index idx_support_ticket_status on app.support_ticket(status);
--------------------------------------------------------------------------------------------
CREATE TABLE app.support_ticket_comment (
  id                  uuid NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY
  ,created_at         timestamptz not null default current_timestamp
  ,updated_at         timestamptz not null default current_timestamp
  ,support_ticket_id  uuid not null references app.support_ticket(id)
  ,resident_id        uuid not null references app.resident(id)
  ,body               text not null
);
create index idx_stc_ticket on app.support_ticket_comment(support_ticket_id);
create index idx_stc_resident on app.support_ticket_comment(resident_id);

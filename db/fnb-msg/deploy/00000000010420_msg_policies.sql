...
alter table msg.topic enable row level security;
    CREATE POLICY view_all_for_tenant ON msg.topic
      FOR SELECT
      USING (jwt.has_permission('p:discussions', tenant_id))
      ;
    CREATE POLICY create_for_tenant ON msg.topic
      FOR INSERT
      WITH CHECK (jwt.has_permission('p:discussions', tenant_id))
      ;
...
alter table msg.message enable row level security;
    ...
    --   FOR ALL
    --   USING (jwt.has_all_permissions('{"p:discussions","p:app-admin"}'::citext[], tenant_id));
    --   ;

    -- CREATE POLICY manage_all_super_admin ON msg.topic
    --   FOR ALL
    --   USING (jwt.has_permission('p:app-admin-super'))
    --   WITH CHECK (jwt.has_permission('p:app-admin-super'))
    --   ;

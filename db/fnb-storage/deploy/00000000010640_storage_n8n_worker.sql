-- Deploy fnb-storage:00000000010640_storage_n8n_worker to pg
...
-------------------------------------- storage_fn.asset_for_scan (asset-scan lookup node)
...
-------------------------------------- storage_fn.stuck_pending_assets (asset-scan-reaper)
-- Returns pending assets older than the threshold whose asset-scan attempt count (rows in
-- n8n.workflow_run) is below the cap and that have no currently-running scan; assets AT the
-- cap are flipped to terminal scan_status='error' for operator review (via the idempotent
-- resolve_asset_scan — same semantics as the retired reaper).
...
-- n8n_worker-only surface — no storage_api wrappers, same posture as resolve_asset_scan.
...
grant usage on schema storage to n8n_worker;
grant usage on schema storage_fn to n8n_worker;
grant execute on function storage_fn.asset_for_scan(uuid) to n8n_worker;
grant execute on function storage_fn.stuck_pending_assets(int, int) to n8n_worker;
grant execute on function storage_fn.resolve_asset_scan(uuid, storage.scan_status, text, text) to n8n_worker;
grant execute on function storage_fn.insert_derived_asset(uuid,uuid,text,text,text,bigint,text,citext[]) to n8n_worker;
grant execute on function storage_fn.add_asset_tags(uuid,citext[]) to n8n_worker;

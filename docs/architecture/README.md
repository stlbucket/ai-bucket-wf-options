...
> **Historical narrative.** These documents predate the GraphQL migration, the ZITADEL cutover,
> the URN registry (which removed the per-module mirror/shadow tables described below), and the
> n8n engine swap (R22). Where anything here differs from `docs/specs/` or the code, **the specs
> win**. Kept for the architectural reasoning and security analysis, which remain instructive.

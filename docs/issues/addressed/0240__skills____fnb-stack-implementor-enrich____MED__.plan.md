> **Moved to addressed/ 2026-07-30** via 0070 (context-architecture refactor). All items done
> except §Details item 4 (Iconify rule) — explicitly deferred to
> `0300__infra_____iconify-rule-verification` by this plan's own text; align the implementor
> `references/` files there when 0300 resolves.
...
> Invoke: `/fnb-stack-spec docs/issues/identified/skill-fnb-stack-implementor-enrich.plan.md`
...
`.agents/skills/fnb-stack-implementor/SKILL.md` (597 lines — largest project skill). It's largely
...
2. **UC13 collision.** The skill defines "UC13 — UTable uses Nuxt UI v4 API" inline, but
   `docs/specs/ui-components-rules.md` reserves UC13 for form validation ("will be formalized as
...
- `grep -n 'useNavRegistry\|nav-register' .agents/skills/fnb-stack-implementor/SKILL.md` → empty.

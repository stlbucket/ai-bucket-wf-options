...
> Invoke: `/fnb-stack-spec docs/issues/identified/skill-fnb-db-designer-jwt-schema.plan.md`
...
`.agents/skills/fnb-db-designer/SKILL.md` (145 lines) documents the JWT/permission helpers as living
...
- `grep -n 'auth\.\(uid\|jwt\|permissions\|has_permission\|enforce_permission\|tenant_id\)' .agents/skills/fnb-db-designer/SKILL.md` → empty (all migrated to `jwt.*`).

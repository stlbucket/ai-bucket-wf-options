...
> Invoke: `/fnb-stack-spec docs/issues/identified/skill-sqitch-expert-corrections.plan.md`
...
`.agents/skills/sqitch-expert/SKILL.md` (275 lines) is stale on three points:
...
3. **Replace the `db-deploy.sh` deployment section** with the real flow: `db/db-config.ts`
   ...
   `docs/specs/monorepo-bootstrap-pattern.md` if it covers deployment, rather than duplicating.
...
- No reference to `db-deploy.sh` or "cross-package … NOT supported" remains
  (`grep -n 'db-deploy.sh\|NOT supported' .agents/skills/sqitch-expert/SKILL.md` → empty).

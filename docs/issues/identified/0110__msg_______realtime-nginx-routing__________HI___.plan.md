...
> Invoke: `/fnb-stack-implementor docs/issues/identified/msg-realtime-nginx-routing.plan.md`
...
2. If broken, the cleanest fix depends on the intended URL contract:
   ...
   - Prefer Option A — it keeps all msg traffic under the one `/msg` prefix the app already declares,
     consistent with the path-based-proxy design in `docs/specs/monorepo-bootstrap-pattern.md`.
...
4. Update `docs/specs/sockets-pattern.md` + `monorepo-bootstrap-pattern.md` (nginx routing) with

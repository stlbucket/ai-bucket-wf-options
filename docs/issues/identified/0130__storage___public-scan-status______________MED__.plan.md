...
> Invoke: `/fnb-stack-implementor docs/issues/identified/storage-public-scan-status.plan.md`
> Coordinate with `docs/specs/asset-storage/` (this touches the active file-upload spec).
...
The asset-storage spec (`docs/specs/asset-storage/README.md`, `endpoint.data.md`) says scanning
...
2. Decide with the user how authenticated/tenant reads should treat non-clean assets: the
   ...
   requirement in `docs/specs/asset-storage/graphql.data.md` now so Phase 5 inherits it.
3. Update `docs/specs/asset-storage/_shared.data.md` invariants to state: "an asset is

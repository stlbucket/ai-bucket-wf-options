...
> Invoke: `/fnb-stack-spec docs/issues/identified/specs-fnb-types-drift.plan.md`
...
1. **`docs/specs/graphql-api-pattern.md`** (Layer 3 barrel section, ~line 136) says the barrel
...
3. **`docs/specs/ui-components-rules.md` UC2** tells the UI to import types from
...
- `grep -n "export \* from './generated" docs/specs/graphql-api-pattern.md` → empty.
- `grep -rn "db-access/src/types/profile-claims\|fnb-graphql-client-api.*ProfileClaims" docs/specs/` → empty.

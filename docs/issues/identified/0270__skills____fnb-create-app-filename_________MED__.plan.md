...
> Invoke: `/fnb-stack-spec docs/issues/identified/skill-fnb-create-app-filename.plan.md`
...
`git ls-files .agents/skills/fnb-create-app/` shows the tracked file is **`skill.md`** (lowercase).
...
1. Case-only rename via git (git doesn't detect case-only renames on case-insensitive FS without
   help):
   ```
   git mv .agents/skills/fnb-create-app/skill.md .agents/skills/fnb-create-app/SKILL.md.tmp
   git mv .agents/skills/fnb-create-app/SKILL.md.tmp .agents/skills/fnb-create-app/SKILL.md
   ```
...
- `git ls-files .agents/skills/fnb-create-app/` → `SKILL.md` (uppercase), no `skill.md`.

...
> Mode 1 reverse-engineer). Invoke: `/fnb-stack-spec docs/issues/identified/0370__specs_____notify-sms-spec-reconcile_______LOW__.plan.md`
> Doc-only — touches `docs/specs/notifications/` only. Never run `git`.
...
D6 note) — annotate it as superseded by `docs/specs/user-invitation/` rather than filling it in.
...
- No `docs/specs/notifications/*` file that describes shipped behavior still says

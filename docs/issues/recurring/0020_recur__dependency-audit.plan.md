...
1. **No floating specifiers** — flag any `"latest"`, `"*"`, or unpinned range across all workspace
   `package.json` files; shared versions are declared once in the pnpm default catalog
   (`pnpm-workspace.yaml`) and referenced as `"catalog:"` (R24). Run `pnpm dep-audit` first —
   it is the hard-fail gate; this checklist is the manual backstop.
...
3. **Direct-dependency rules honored** — layers/apps that use `@nuxt/ui` types declare
   `"@nuxt/ui": "catalog:"` directly (pnpm does not hoist transitive packages); each Nuxt app declares

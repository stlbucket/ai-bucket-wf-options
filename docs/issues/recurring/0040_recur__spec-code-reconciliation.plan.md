...
Periodically, and after any architecture change — reconcile `docs/specs/` (pattern files +
...
1. **Pattern files vs code** — `graphql-api-pattern.md`, `package-layers-pattern.md`,
   ...
   `withClaims`).
2. **global-rules R1–R24** — each rule still matches reality; no rule contradicts another or the
   pattern files.
...
4. **Single-description invariant (R21)** — the stack is described in exactly three places
   (global rules, the pattern file, the two orchestrator skills — including their `references/`
   files) with no inline duplication drift.

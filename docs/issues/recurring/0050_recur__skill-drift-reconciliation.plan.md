...
re-verify every `.agents/skills/*/SKILL.md` **and its `references/*.md` files** against the
live code. This is the sweep behind
...
3. **Package/db lists** — the package list (ten + `game-engines`) and db-package list
   (thirteen) are complete and current wherever cited.
4. **Version pins** — Nuxt UI v4 API, the `"@nuxt/ui": "catalog:"` direct-dependency rule
   (R24), etc.

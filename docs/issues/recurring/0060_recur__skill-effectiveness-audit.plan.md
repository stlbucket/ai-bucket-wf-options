...
Audit every `.agents/skills/*/SKILL.md` plus `.agents/skills/skill-map.md`:
...
1. **Trigger clarity** — each skill's frontmatter `description` states *when to invoke it* with
   ...
   deliberate overlap states which one wins (e.g. the skill-map tombstone routing old
   graphile-worker triggers to `n8n-cli`).
...
7. **Retirement hygiene** — retired skills are clearly marked LEGACY at the top, state *why*
   and *when*, and route their old triggers to the replacement. Fully dead ones get deleted,
   leaving a dated skill-map tombstone (precedent: `graphile-worker-expert`, flagged by this
   audit and deleted 2026-07-30).

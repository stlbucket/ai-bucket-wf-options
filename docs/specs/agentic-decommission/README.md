...
The reverse of `docs/specs/agentic-workflow-engine/decommission.data.md`: that spec retired
...
(`docs/specs/n8n-parallel-engine/`). Three responsibilities still live on the agentic engine
(`apps/agent-app`, R22):
...
| Agentic spec + skill | **Delete outright** — `docs/specs/agentic-workflow-engine/` and the `claude-agent-sdk` skill | User decision 2026-07-20. Git retains the history; the tree carries no dead agentic docs |
...
- [ ] **Phase 5 — R21 propagation + final verification**: global-rules R22 rewrite (sole n8n
      ...
      **delete `docs/specs/agentic-workflow-engine/`**, repoint the `asset-storage` superseded
...
| Rename `docs/specs/n8n-parallel-engine/` → `n8n-engine` | Churn for no behaviour change; a Status note that it is now the sole engine is enough |

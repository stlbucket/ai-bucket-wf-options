...
`docs/specs/agentic-workflow-engine/decommission.data.md` (which retired graphile-worker),
...
| `docs/specs/agentic-workflow-engine/` (whole dir) | user decision — delete outright (R21) |
| `.agents/skills/claude-agent-sdk/` (whole skill) | user decision — delete + remove from `skill-map.md` (R21) |
...
- `CLAUDE.md`: remove the `agent-app` apps-table row; drop `fnb-agent` from the db list **and its
  ...
  engine").
- `.agents/skills/skill-map.md`: remove the `claude-agent-sdk` registration; `graphile-worker-expert`
  is already legacy; rewrite any `fnb-stack-implementor` / `fnb-stack-spec` agentic references to
  the n8n engine.
- `docs/specs/asset-storage/asset-scan-workflow.data.md`: repoint the superseded-by pointer
  ...
  `infrastructure.md`.
- `docs/specs/n8n-parallel-engine/README.md` + `_shared.data.md`: add a Status note that n8n is
...
- `docs/specs/graphql-api-app/`: any `worker-pattern.md` / server-pattern agentic references

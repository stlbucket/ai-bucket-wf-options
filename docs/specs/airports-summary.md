...
`docs/specs/tenant-app/datasets/airports/`.
...
`.agents/skills/airports-expert/SKILL.md`, and registered it in `skill-map.md` and the `/fnb`
...
`docs/specs/tenant-app/datasets/airports/` mirroring the breweries spec **file for file**:
...
```
docs/issues/identified/0010__loc_______airports-dataset________________MED__.plan.md
```
...
**Where to look:**
- Dataset facts / access model → `.agents/skills/airports-expert/SKILL.md`
- Design + decisions + verified results → `docs/specs/tenant-app/datasets/airports/`
  (start at `README.md`)
- The sequenced plan with its post-implementation addendum →
  `0010__loc_______airports-dataset________________MED__.plan.md` under `docs/issues/`

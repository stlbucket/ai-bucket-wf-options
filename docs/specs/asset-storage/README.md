...
> contract: `docs/specs/urn-registry/stacking-v2.data.md`. `context`/`owning_entity_id`
...
> `docs/specs/agentic-decommission/asset-scan.workflow.data.md`). The upload endpoint fires a
...
`docs/issues/addressed/0350__storage___asset-image-thumbnails__________LOW__.plan.md`. All SQL is in-place edits to existing
...
`docs/issues/in-flight/0010__storage___asset-detail-page…plan.md` (staged §1 DB → §6 spec reconcile).
...
plans under `docs/issues/identified/`:
...
- ~~How tenant-app later consumes storage-layer components for todo/ticket embedding~~ —
  **resolved 2026-07-09 (issue 0330):** tenant-app `extends` `@function-bucket/fnb-storage-layer`
  ...
  `docs/specs/tenant-app/tools/todo/[id].*`).

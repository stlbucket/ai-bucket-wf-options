...
> docs/specs/db-introspect/README.md` — the implementor derives the
> `docs/issues/` plan file (R23) from the task list below, then executes it.

## Status
Implemented 2026-07-24 — `scripts/db-introspect.ts` + `pnpm db-introspect` + hand-written
`docs/db-structure/index.html`. Plan:
`docs/issues/…/0010__db________db-introspect-structure-docs____MED__.plan.md`. One contract

...
The dev stack (`docker-compose.yml`, `docs/specs/monorepo-bootstrap-pattern.md`) is built for
...
`docs/specs/deployment/dev-caddy-migration/`). The prod broker does three jobs the dev one
...
1. **Site admin** — `POST https://<domain>/auth/api/setup/initialize` with
   ...
   first-run-setup endpoint (`docs/specs/first-run-setup/`): ZITADEL human user first

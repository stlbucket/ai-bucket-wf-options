...
Plan (closed): `docs/issues/addressed/0350__auth______zitadel-login-provider__________HI___.plan.md`
...
- ZITADEL cannot live under an nginx **path prefix** (the issuer must own its origin; console
  ...
  upstream — see `.agents/skills/zitadel-expert/references/self-hosting.md`.
...
- **`zitadel-seed`** one-shot (`docker/zitadel/seed.mjs` on `node:22-alpine`, PAT auth,
  ...
  **Also applies instance branding** (`ensureBranding()`, plan 0500 / spec
  `docs/specs/brand-identity/`): the fnb brand mark on the hosted login via the **instance**
...
`monorepo-bootstrap-pattern.md` (services), `docs/specs/auth-app/login.*`,

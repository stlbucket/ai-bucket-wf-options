...
> `docs/specs/game-server/` (README + `_shared.data.md` + `infrastructure.md` +
...
`docs/issues/in-flight/0370__app_______game-server-checkers____________MED__.plan.md` holds
...
- [x] **Phase 4 — E2E + propagation**: R21 propagation done — parent `game-server/README.md`
      (battleship + checkers playable), parent `coming-soon.*` (Tic-Tac-Toe only), `docs/specs/STATUS.md` (was the `fnb-stack-spec`
      Implemented Modules table) `games` row; parent `game-event.workflow.data.md` (generalized agent

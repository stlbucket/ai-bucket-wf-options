...
Barrel `src/index.ts`: one `export * from './composables/use{Domain}'` per composable, plus the
handful of generated urql hooks the UI needs as values. It does **not** `export *` the generated
module — generated types are internal to this package, bridged to `fnb-types` shapes by mappers
(`src/mappers/<entity>.ts`, one pure `to<Entity>(fragment)` per entity — global-rules R3). A
missing barrel line is a hard ESM startup crash (`does not provide an export named 'X'` at
`dist/index.js`), not a build error.
...
`packages/fnb-types/src/profile-claims.ts` (NOT generated — see global-rules R3):
```ts
{
...
the post-commit asset-scan webhook POST to n8n — R22). Both use `withClaims` (2-arg) from
`packages/db-access`:
...
**OTP quick-login** (`docs/specs/otp-login/`) adds a second way to *mint* a session in the
...
**First-run setup** (`docs/specs/first-run-setup/`) adds two more pre-claims functions in the
...
app — but its endpoints are the GraphQL transport, not per-feature REST routes. Workflows run on
**n8n** (R22, the sole engine); graphql-api-app's only workflow surface is the
`triggerWorkflow` extendSchema plugin (`server/graphile/trigger-workflow.plugin.ts`): claims 401
gate → `WORKFLOW_REGISTRY` permission map → `X-Fnb-Webhook-Secret` POST to
`${N8N_INTERNAL_URL}/webhook/<key>`, respond-immediately (no runId).

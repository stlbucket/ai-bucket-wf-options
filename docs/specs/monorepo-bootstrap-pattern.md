...
only; full contract in `docs/specs/future-auth/zitadel-login-pattern.md` — do not restate it
here):
...
door `infra/docker/Caddyfile` (spec `docs/specs/deployment/dev-caddy-migration/README.md`):
```yaml
...
front door `infra/docker/Caddyfile`; migration spec `docs/specs/deployment/dev-caddy-migration/`).
...
keys mitigate this (both ignored by `nuxt build`); see spec `docs/specs/dev-startup-performance/`.
...
The **n8n workflow engine** (R22, the sole engine; specs `docs/specs/n8n-parallel-engine/` +
`docs/specs/agentic-decommission/` — do not restate them here) is three compose services,

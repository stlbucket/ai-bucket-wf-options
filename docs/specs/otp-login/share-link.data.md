...
- **Delivery** rides the existing `send-notification` n8n workflow (R22) via the house
  ...
  `docs/specs/notifications/` (webhook vs registry): one send per (resident × ticked channel),
...
- [ ] `send_deep_link` trigger boundary (webhook vs `triggerWorkflow` registry) + the
      `deep-link-share` template shape — resolve against `docs/specs/notifications/` at plan time.

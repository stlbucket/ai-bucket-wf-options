...
`graphql-client-api`. See `docs/specs/graphql-api-pattern.md` for the data stack.)
...
(spec `docs/specs/game-server/`); its build output is embedded verbatim into the `game-event`
...
enforcement (`pnpm dep-audit`): `docs/specs/workspace-dependency-integrity-pattern.md`.
...
- `src/use-residency-switcher.ts` — `useResidencySwitcher()`: derives the workspace-switcher
  ...
  `switchResidency` (spec: `docs/specs/workspace-switcher/`).
...
**Build:** Vite (`vite-plugin-dts`) to `dist/`. See also `docs/specs/graphql-api-pattern.md`
(the data stack) and `docs/specs/graphql-api-app/graphql-client-api-package.md`.
...
`docs/specs/workspace-dependency-integrity-pattern.md`): it has its own `tsconfig.json`
...
| `components/ModuleNavSection.vue` | Renders one nav section from claims `modules`; expanded form is a per-section `UCollapsible` disclosure (spec `docs/specs/nav-collapsible-sections/`), icon-only in the collapsed rail |
| `components/WorkspaceSwitcher.vue` | Sidebar current-tenant trigger + residency-tree `UModal`/`UTree`; mounted in `AppNav` + `AppNavMobile` (spec: `docs/specs/workspace-switcher/`) |
| `components/FunctionBucketMark.vue` | Inline-SVG brand mark (bucket glyph + optional ƒb monogram; `primary`/`secondary` fill) |
| `components/OtpSessionBanner.vue` | Temporary-session countdown banner for OTP quick-login; polls auth-app `/api/session-info` (pre-claims, same-origin), renders only for `authMethod='otp'` (spec `docs/specs/otp-login/`) |
...
| `composables/useAppNav.ts` | Nav state from claims `modules` + mobile-drawer open/close + whole-nav-collapse + **per-section collapse** (top-3-open default, localStorage `fnb:nav-section:*`, spec `docs/specs/nav-collapsible-sections/`) |
...
Full spec: `docs/specs/asset-storage/`.
...
for the game server (spec `docs/specs/game-server/`); no app-layer UI (the battleship

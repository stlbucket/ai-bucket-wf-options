...
`docs/specs/tenant-app/datasets/breweries/`). OurAirports has **no API**: the entire
dataset ships as seven nightly-refreshed CSV files on GitHub Pages; the sync downloads six of
them whole and upserts locally (see `.agents/skills/airports-expert/SKILL.md` — the recon
...
Same stack as breweries (see `docs/specs/tenant-app/datasets/breweries/_shared.data.md`):

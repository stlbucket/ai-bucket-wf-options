...
> tables carry a generated `urn` column. Authoritative contract: `docs/specs/urn-registry/`
...
max `per_page=200`. See `.agents/skills/breweries-expert/SKILL.md` for endpoint/field details.
...
Same stack as `loc` (see `docs/specs/tenant-app/loc/_shared.data.md`):
...
`docs/issues/identified/0010__loc_______breweries-dataset_______________MED__.plan.md`):
...
- [x] 2. `wf_api.queue_workflow` carries **no permission gate** (SECURITY DEFINER, granted to
      ...
      **User decision:** no guard in this feature; the sync trigger ships **UI-gated only**
      ...
      `docs/issues/identified/0030__wf________wf-rls-missing__________________CRT__.plan.md`

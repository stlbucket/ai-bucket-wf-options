...
- **URN registry** (`docs/specs/urn-registry/`) — the poll is a registered business entity
  (`res_fn.register_resource(id, tenant, 'poll', 'poll', resident)`), exactly like `todo.todo`.
- **Discussion by subject URN** (`docs/specs/urn-registry/stacking-v2.data.md`) — the optional
...
- **OTP deep-link login** (`docs/specs/otp-login/`) — polls are the module the otp-login spec

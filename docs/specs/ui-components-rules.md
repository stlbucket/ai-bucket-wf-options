...
### UC15 — UTable uses the Nuxt UI v4 (TanStack) column API exclusively
Never use the v3 `{ key, label }` column shape. (Previously stated inline in
`fnb-stack-implementor` as "UC13" — renumbered here; UC13 stays reserved for form validation.)

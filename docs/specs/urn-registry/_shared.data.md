...
- `.agents/skills/fnb-db-designer/SKILL.md` — package list, UUIDv7 default, "registered
...
- `.agents/skills/fnb-stack-implementor/SKILL.md` + `docs/specs/graphql-api-pattern.md` —
...
- `docs/specs/package-layers-pattern.md` — `fnb-types` wording ("type-only, zero runtime
  values" → allows the pure `parseUrn`/`formatUrn`/`isUrn` helpers).
- `.agents/skills/skill-map.md` — no new skill; fnb-res is owned by `fnb-db-designer`.
- Module specs that document the mirrors (`docs/specs/**` msg/todo/loc/asset-storage

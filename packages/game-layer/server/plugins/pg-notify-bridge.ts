...
const logger = createConsola({ tag: 'game-bridge' })

// msg-layer mirror (sockets-pattern; deep-refs d5/d6): one dedicated pg.Client (not a
// pool — LISTEN state is per-connection), a hand-rolled channelPeers map, and the h3
// websocket.resolve override (the auth middleware otherwise intercepts WS upgrade
// resolution before the router — the fix is load-bearing, do not remove).
...
export default defineNitroPlugin(async (nitro) => {
  ...
  // Bug: h3's createResolver finds the auth middleware (registered at the base path,
  // non-lazy, no __resolve__) before the router, so websocket hooks are never reached.
